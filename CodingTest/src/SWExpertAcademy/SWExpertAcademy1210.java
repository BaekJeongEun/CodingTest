...
public class SWExpertAcademy1210 {
	...
	public static void main(String[] args) {
		...
		for (int tc = 1; tc <= TC; tc++) {
			sc.nextInt(); // tc번호가 굳이 입력으로 들어옴. 안쓸거지만 받긴 해야함

			ans = -1;
			map = new int[100][100];
			nowi = nowj = nexti = nextj = -1; // 유효하지 않은 값으로 초기화하고 쓰기
			...
			while (true) {
				map[nowi][nowj] = 9;
				for (int d = 0; d < 3; d++) { // 3개 방향중 이동가능한 옆칸 찾아서 현재좌표 변경
					nexti = nowi + di[d];
					nextj = nowj + dj[d];
					...
					if (nexti >= 0 && nexti < 100 && nextj >= 0 && nextj < 100 && map[nexti][nextj] == 1) {
						nowi = nexti;
						nowj = nextj;
						break; // 좌, 우, 상 순서로 탐색중. 좌, 우 중에 이동가능하면 바로 이동하고 위로는 안가야함.
					}
				}
				if(nowi == 0) { // 꼭대기 도착!
					ans = nowj; // 현재 열번호가 답!
